...
class BitArray(object):
    __slots__ = ('length', 'data', '_set_idx')
    ...
    def left_most_bit(self) -> int:
        return self._set_idx
    ...
    def __setitem__(self, key: int, value: bool) -> None:
        ...
        if value and (self._set_idx < 0 or key < self._set_idx):
            self._set_idx = key
        if not value and self._set_idx == key:
            # curr cleared
            self._set_idx = -1
            for i in range(key // 8, len(self.data)):
                item = self.data[i]
                j = 0
                while item != 0:
                    item >>= 1
...
class SchedQueue:
    def __init__(self, max_prio):
        ...
        self.active_barray = BitArray(max_prio)
        self.active_queue = [[] for _ in range(max_prio)]
        self.expired_barray = BitArray(max_prio)
        self.expired_queue = [[] for _ in range(max_prio)]

    def push_to_active(self, job, prio):
        self.active_barray[prio] = True
        self.active_queue[prio].append(job)

    def push_to_expired(self, job, prio):
        self.expired_barray[prio] = True
        self.expired_queue[prio].append(job)

    def has_job(self):
        prio = self.active_barray.left_most_bit()
        return prio >= 0
    
    def print(self):
        print("[ACTIVE]-")
        prefix = " " * 8
        for i in range(len(self.active_barray)):
            if self.active_barray[i]:
                print(prefix + "|" + " {:2d}".format(i) + ":" + ",".join(self.active_queue[i]))
        print("[EXPIRED]-")
        prefix = " " * 8
        for i in range(len(self.expired_barray)):
            if self.expired_barray[i]:
                print(prefix + "|" + " {:2d}".format(i) + ":" + ",".join(self.expired_queue[i]))

    def pop(self):
        prio = self.active_barray.left_most_bit()
        if prio < 0:
            tmp = self.active_barray
            self.active_barray = self.expired_barray
            self.expired_barray = tmp

            tmp = self.active_queue
            self.active_queue = self.expired_queue
            self.expired_queue = tmp
        
        prio = self.active_barray.left_most_bit()
        if prio >= 0:
            job = self.active_queue[prio].pop()
            if len(self.active_queue[prio]) == 0:
                self.active_barray[prio] = False
            return job
        else:
            return None
#
...
class Scheduler:
    def __init__(self, job_list, per_cpu_queues, affinity, peek_interval,
        ...
        if self.per_cpu_queues:
            for cpu in range(num_cpus):
                self.per_cpu_sched_queue[cpu] = SchedQueue(self.max_prio)
                ...
                for cpu in range(num_cpus):
                    ...
                    for job_name in jobs_not_assigned:
                        if len(self.jobs[job_name].affinity) == 0 or cpu in self.jobs[job_name].affinity:
                            self.per_cpu_sched_queue[cpu].push_to_active(job_name, random.randint(0, self.max_prio - 1))
            ...
            for cpu in range(num_cpus):
                print('Scheduler CPU %d queue: ' %
                      (cpu))
                self.per_cpu_sched_queue[cpu].print()
            print('')

        else:
            # assign them all to same single queue
            self.single_sched_queue = SchedQueue(self.max_prio)
            for job_name in self.job_name_list:
                self.single_sched_queue.push_to_active(job_name, random.randint(0, self.max_prio - 1))
            for cpu in range(num_cpus):
                self.per_cpu_sched_queue[cpu] = self.single_sched_queue

            print('Scheduler central queue: ')
            self.single_sched_queue.print()
    ...
    def handle_one_interrupt(self, interrupt, cpu):
        ...
        if interrupt and self.sched_state[cpu] == self.STATE_RUNNING:
            ...
            self.per_cpu_sched_queue[cpu].push_to_expired(job_name, random.randint(0, self.max_prio - 1))
        return
    ...
    def get_job(self, cpu, sched_queue):
        ...
        if len(self.jobs[job_name].affinity) == 0 or cpu in self.jobs[job_name].affinity:
            ...
            raise NotImplementedError("DO NOT support affinity with a single global sched queue")
    ...
    def steal_jobs(self):
        ...
        if self.system_time > 0 and self.system_time % self.peek_interval == 0:
            for cpu in range(self.num_cpus):
                if not self.per_cpu_sched_queue[cpu].has_job():
                    ...
                    job_name = self.per_cpu_sched_queue[other_cpu].pop()
                    if not job_name:
                        continue
...
parser.add_option('-c', '--compute',     default=True, help='compute answers for me',
