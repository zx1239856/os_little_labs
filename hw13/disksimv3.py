...
class Disk:
    ...
    def PlanSeek(self, track, extra=0):
    ...
    def DoCSCAN(self, rList):
        ...
        global_min_idx = -1
        global_min_blk = -1
        global_min_track = MAXTRACKS

        for (block, index) in rList:
            if self.requestState[index] == STATE_DONE:
                continue
            track = self.blockToTrackMap[block]
            dist = int(track - self.armTrack)
            if dist >= 0 and dist < min_dist:
                min_dist = dist
                min_idx = index
                min_blk = block
            if track < global_min_track:
                global_min_track = track
                global_min_idx = index
                global_min_blk = block
        extra = 0
        if min_idx == -1:
            min_idx = global_min_idx
            min_blk = global_min_blk
            extra += len(self.tracks) - 1
        assert(min_blk != -1)
        assert(min_idx != -1)
        return (min_blk, min_idx, extra)
    ...
    def DoCLOOK(self, rList):
        ...
        for (block, index) in rList:
            ...
            if track < min_track:
                ...
                min_idx = index
            dist  = int(track - self.armTrack)
            if dist >= 0 and dist < min_positive_dist:
                min_positive_dist = dist
                min_positive_idx = index
                min_positive_blk = block
        if min_positive_idx != -1:
            min_blk, min_idx = min_positive_blk, min_positive_idx

        assert(min_blk != -1)
        assert(min_idx != -1)
        return (min_blk, min_idx)
    ...
    def GetNextIO(self):
        ...
        if self.policy == 'FIFO':
            ...
            (self.currentBlock, self.currentIndex, extra) = self.DoCSCAN(self.requestQueue[0:self.GetWindow()])
        else:
            print('policy (%s) not implemented' % self.policy)
            sys.exit(1)

        # once best block is decided, go ahead and do the seek
        self.PlanSeek(self.blockToTrackMap[self.currentBlock], extra)
